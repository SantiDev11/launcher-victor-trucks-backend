"""
GRÃFICOS VICTORTRUCKS - Production API Server
...
def get_current_user(authorization: Optional[str] = Header(None)):
    ...
    if not user:
        raise HTTPException(status_code=401, detail="SesiÃ³n invÃ¡lida o expirada. Inicia sesiÃ³n nuevamente.")
...
def require_admin(current_user: dict = Depends(get_current_user)):
    ...
    The role is verified server-side from the database â€” never from client claims.
...
class RegisterRequest(BaseModel):
    ...
    def validate_password(cls, v):
        ...
        if not re.search(r'[A-Za-z]', v):
            raise ValueError('La contraseÃ±a debe contener al menos una letra')
...
class ChangePasswordRequest(BaseModel):
    ...
    def validate_password(cls, v):
        ...
        if not re.search(r'[^A-Za-z0-9]', v):
            raise ValueError('La contraseÃ±a debe contener al menos un carÃ¡cter especial (!@#$%^&* etc)')
        return v
...
def register(req: RegisterRequest):
    if not req.username or not req.password:
        raise HTTPException(status_code=400, detail="Usuario y contraseÃ±a requeridos")
...
def login(req: LoginRequest):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, username, password_hash, password_salt, role, is_active, must_change_password FROM users WHERE username = ?", (req.email,))
...
def change_password(req: ChangePasswordRequest):
    if not req.username or not req.current_password or not req.new_password:
        raise HTTPException(status_code=400, detail="Usuario, contraseÃ±a actual y nueva contraseÃ±a son requeridos")
    ...
    if not user:
        conn.close()
        raise HTTPException(status_code=401, detail="Usuario o contraseÃ±a actual incorrectos")
    ...
    if req.current_password == req.new_password:
        conn.close()
        raise HTTPException(status_code=400, detail="La nueva contraseÃ±a debe ser distinta a la actual")
...
def get_mod(mod_id: int, current_user: dict = Depends(get_current_user)):
    ...
    if not r:
        raise HTTPException(status_code=404, detail="Mod grÃ¡fico no encontrado")
...
def download_mod(mod_id: int, range_header: Optional[str] = Header(None, alias="Range"), current_user: dict = Depends(get_current_user)):
    ...
    if range_header and range_header.startswith("bytes="):
        ...
        try:
            ...
            raise HTTPException(status_code=416, detail="Rango invÃ¡lido")
...
def create_mod_manual(req: CreateModRequest, _admin: dict = Depends(require_admin)):
    ...
    if not req.title or not req.version:
        raise HTTPException(status_code=400, detail="TÃ­tulo y versiÃ³n requeridos")

    category = "GrÃ¡ficos generales"
    ...
    return {
        "success": True,
        "mod_id": mod_id,
        "message": f"Mod '{req.title}' creado. Agrega el archivo cuando estÃ© disponible."
    }
...
async def upload_mod(
    ...
    return {
        ...
        "message": "Mod subido exitosamente al catÃ¡logo"
    }
...
def update_user(user_id: int, req: AdminUserUpdateRequest, _admin: dict = Depends(require_admin)):
    ...
    if req.role is not None:
        ...
        if user_row[2] == "admin" and req.role.lower() != "admin":
            ...
            if admin_count <= 1:
                conn.close()
                raise HTTPException(status_code=400, detail="No se puede quitar el rol ADMIN al Ãºnico administrador")
    ...
    for field, value in req.model_dump(exclude_none=True).items():
        if field == "title":
            ...
            if value not in CATEGORIES:
                conn.close()
                raise HTTPException(status_code=400, detail=f"CategorÃ­a invÃ¡lida: {value}")
...
def delete_mod(mod_id: int, _admin: dict = Depends(require_admin)):
    ...
    return {"success": True, "message": "Mod eliminado del catÃ¡logo"}
...
def unhide_mod(mod_id: int, _admin: dict = Depends(require_admin)):
    ...
    conn.commit()
    conn.close()
    return {"success": True, "message": "Mod visible nuevamente en el catÃ¡logo"}
...
if __name__ == "__main__":
    ...
    print("=" * 62)
    print("  GRÃFICOS VICTORTRUCKS - Servidor API central")
    print("=" * 62)
